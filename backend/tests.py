...
class APITestCase(unittest.TestCase):
    ...
    def test_list_create(self):
        response = self.client.post('/lists', json=self.__class__.list)
        self.assertEqual(response.status_code, 201)
        self.__class__.list_id = response.get_json().get('id')

    def test_list_create_invalid(self):
        response = self.client.post('/lists', json={'test': "test", 'color': 'red'})
    ...
    def test_list_read(self):
        response = self.client.get(f'/lists/{self.__class__.list_id}')
        self.assertEqual(response.status_code, 200)
    ...
    def test_task_create(self):
        response = self.client.post('/tasks', json=self.__class__.task)
        self.assertEqual(response.status_code, 201)
        self.__class__.task_id = response.get_json().get('id')

    def test_task_create_invalid(self):
        response = self.client.post('/tasks', json={'test': "test"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_task_read(self):
        response = self.client.get(f'/tasks/{self.__class__.task_id}')
        self.assertEqual(response.status_code, 200)
